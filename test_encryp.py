...
if len(text_aux) % max_len == 0:
    rang = len(text_aux) // max_len
else:
    rang = (len(text_aux) // max_len) + 1
dig_lis = [text_aux[max_len * idx: max_len * (idx + 1)] for idx in range(rang)]
...
bin_k2 = str(bin(k2))
bin_k2 = bin_k2[2:]
# bin_k2 = [int(num) for num in bin_k2]
...
for idx in range(len(int_lis)):    
    powers = power_tower(int_lis[idx], k1, len(bin_k2))
