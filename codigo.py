...
class Mensaje:
    ...
    def cifrar_mensaje(self, e):
        ...
        enteros_cifrados = [self._calcular_Pe_mod_p(
            num, e, self.p) for num in enteros if True]
    ...
    def decifrar_texto(self, d):
        ...
        lista_enteros_cifrados.append(grupo)
        try:
            enteros_cifrados = [int(num) for num in lista_enteros_cifrados]
        except ValueError:
            print('Valores ingresados no son correctos!')

        enteros = [self._calcular_Pe_mod_p(
            num, d, self.p) for num in enteros_cifrados if True]
    ...
    def _int_a_str(self, texto_decifrado):
        ...
        texto_final = ''
        letras_num = [texto_decifrado[i*2: (i + 1)*2]
                      for i in range(longitud) if True]
